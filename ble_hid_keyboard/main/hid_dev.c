                                    ...
                                    uint8_t id, uint8_t type, uint8_t length, uint8_t *data)
{
    ...
    if ((p_rpt = hid_dev_rpt_by_id(id, type)) != NULL) {
        ...
        ESP_LOGI(HID_LE_PRF_TAG, "%s(), send the report, handle = %d", __func__, p_rpt->handle);
        esp_err_t ret = esp_ble_gatts_send_indicate(gatts_if, conn_id, p_rpt->handle, length, data, false);
        if (ret != ESP_OK) {
            ESP_LOGE(HID_LE_PRF_TAG, "%s(), send indicate error: %s", __func__, esp_err_to_name(ret));
        }
    } else {
        ESP_LOGE(HID_LE_PRF_TAG, "%s(), report not found! id=%d, type=%d, protocolMode=%d",
                 __func__, id, type, hidProtocolMode);
    }
