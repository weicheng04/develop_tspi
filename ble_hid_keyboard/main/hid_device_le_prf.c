									...
									esp_ble_gatts_cb_param_t *param)
{
    switch(event) {
        ...
        case ESP_GATTS_WRITE_EVT: {
            // Check if write is for the NUS string service
            if (ble_string_svc_handle_write_evt(gatts_if, param)) {
                break;
            }
            esp_hidd_cb_param_t cb_param = {0};
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            // Check if this is the NUS string service attr table
            if (ble_string_svc_handle_create_evt(param)) {
                break;
            }
            ...
            if (param->add_attr_tab.num_handle == HIDD_LE_IDX_NB &&
                ...
                // HID service is ready, now create the NUS string service
                ble_string_svc_create(gatts_if);
            } else {
